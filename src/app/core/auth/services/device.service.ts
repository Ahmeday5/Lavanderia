 ...
 * the backend can use for session/audit binding on login and refresh calls.
 *
 * `deviceId` is generated once and persisted. Clearing localStorage rotates
 * it — that's intentional (treated as a new device on the server). Drop this
 * service (and its fields from the login/refresh payloads) entirely if your
 * backend doesn't do device binding.
 */
